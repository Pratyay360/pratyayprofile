...
import { AUTH_COOKIE, clearAuthCookie, createPocketBaseClient, setAuthCookie, verifySuperuserToken } from '$lib/server/admin-auth';

export const load = async ({ cookies }) => {
	const token = cookies.get(AUTH_COOKIE);
	if (!token) return {};

	const pb = await verifySuperuserToken(token);
	if (!pb) {
		...
		try {
			...
			if (!pb.authStore.isSuperuser) {
				return fail(403, {
					...
					email,
					otpId,
					error: 'Authenticated account is not a superuser.'
				});
			}
