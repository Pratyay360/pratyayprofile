...
<main class="bg-background flex min-h-screen items-center justify-center p-4">
	...
	<Card class="w-full max-w-2xl border-slate-200 shadow-xl dark:border-slate-700">
		<CardHeader class="pb-4">
			<CardTitle class="text-foreground text-center text-3xl font-bold">Contact Us</CardTitle>
		...
		<CardContent>
			<form class="space-y-6" on:submit|preventDefault={submitRequest}>
				<div>
					<label for="userName" class="text-foreground text-sm font-medium">Name</label>
					<Input
						id="userName"
						name="Name"
				...
				<div>
					<label for="userEmail" class="text-foreground text-sm font-medium">Email</label>
					<Input
						id="userEmail"
						name="Email"
						type="email"
				...
				<div>
					<label for="userMessage" class="text-foreground text-sm font-medium">Message</label>
					<Textarea
						id="userMessage"
						name="Message"
						rows={5}
				...
				<Button type="submit" variant="outline" disabled={submissionStatus}>
					{#if submissionStatus}
						Submitting...
					{:else}
						Submit Message
