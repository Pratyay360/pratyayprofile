<script lang="ts">
	...
	onMount(async () => {
		try {
			const records = await pb.collection('blogs').getFullList<RecordModel>({ sort: '-created' });
...
<main class="bg-background min-h-screen px-4 py-24">
	<h1 class="mb-16 text-center text-4xl font-bold tracking-wider md:text-5xl">Blogs.</h1>
